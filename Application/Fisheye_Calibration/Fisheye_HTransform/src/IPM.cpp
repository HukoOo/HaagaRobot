...
{
	assert( m_origPoints.size() == m_dstPoints.size()  && "Orig. points and Dst. points must vectors of 4 points" );
...
void IPM::drawPoints( const std::vector<cv::Point2f>& _points, cv::Mat& _img ) const
{
	...
	{		
		if(i>0)
			line(_img, Point(static_cast<int>(_points[i].x), static_cast<int>(_points[i].y)), Point(static_cast<int>(_points[i-1].x), static_cast<int>(_points[i-1].y)), CV_RGB(255, 0, 0), 1);
		else
			line(_img, Point(static_cast<int>(_points[0].x), static_cast<int>(_points[0].y)), Point(static_cast<int>(_points[_points.size()-1].x), static_cast<int>(_points[_points.size() - 1].y)), CV_RGB(255, 0, 0), 1);
		circle(_img, Point(static_cast<int>(_points[i].x), static_cast<int>(_points[i].y)), 2, CV_RGB(255, 255, 0), -1);
	}
}
